...
export function targetsForElement(element: LayoutElement): AnimationTargetDescriptor[] {
  ...
  if (element.kind === "line-chart") return [
    { ...base, id: `${element.id}:axis`, phase: 0, kind: "path-draw", pathLength: 310 },
    { ...base, id: `${element.id}:labels`, phase: 1, kind: "text-reveal", graphemes: element.payload.labels.reduce((sum, label) => sum + countGraphemes(label), 0) },
    ...element.payload.series.map((series, index) => ({ ...base, id: `${element.id}:series:${series.id}`, phase: index + 2, kind: "path-draw" as const, pathLength: 420 })),
  ];
