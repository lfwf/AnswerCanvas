...
describe("text placement", () => {
  it("snaps ruled handwritten baselines only when requested", () => {
    const placement = resolveTextPlacement({ id: "line", kind: "text", order: 1, x: 20, y: 40, width: 300, text: "文字", style: { fontSize: 18, lineHeight: 29, snapToRule: true } }, ruledPaper);
  ...
  it("keeps authored ruled coordinates unless snapping is explicitly enabled", () => {
    expect(resolveTextPlacement({ id: "label", kind: "text", order: 1, x: 20, y: 40, width: 100, text: "标签", style: { nudgeX: 2, nudgeY: -3 } }, ruledPaper)).toEqual({ left: 22, top: 37, lineHeight: undefined });
  });

  it("never snaps dotted paper text to a ruled baseline", () => {
    expect(resolveTextPlacement({ id: "label", kind: "text", order: 1, x: 20, y: 40, width: 100, text: "标签", style: { nudgeY: 2, snapToRule: true } }, dottedPaper)).toEqual({ left: 20, top: 42, lineHeight: undefined });
  });
