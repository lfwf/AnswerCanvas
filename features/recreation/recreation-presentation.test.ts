...
const scene: RecreationScene = {
  ...
  elements: [
    ...
    { id: "focus", kind: "view", order: 3, mode: "focus", phase: "analysis", targetIds: ["sentence"], elementOpacity: { notes: 0.35 }, dimOpacity: 0.1 },
    { id: "new-analysis", kind: "text", order: 4, x: 10, y: 90, width: 180, text: "new analysis" },
    { id: "restore", kind: "view", order: 5, mode: "restore", phase: "final" },
  ],
};
...
describe("presentationOpacityFor", () => {
  it("preserves focus targets, applies explicit lecture opacity, and keeps static paper structure", () => {
    const progress = { focus: 1 };
    expect(presentationOpacityFor(sentence, scene, progress)).toBe(1);
    expect(presentationOpacityFor(notes, scene, progress)).toBeCloseTo(0.35);
  ...
  it("interpolates lecture transitions instead of popping visibility", () => {
    expect(presentationOpacityFor(notes, scene, { focus: 0.5 })).toBeCloseTo(0.675);
  });
