...
export function LineChart({ element, progress }: { element: ChartLayoutElement; progress: Record<string, number> }) {
  ...
  const pathFor = (points: number[], seed: string) => roughPolylinePath(points.map((point, index) => ({ x: xFor(index, points.length), y: yFor(point) })), seed, 2.8);
  ...
  return (
    <svg className="chart-element handwritten-element" style={{ left: element.box.x, top: element.box.y, width: element.box.width, height: element.box.height }} viewBox={`0 0 ${element.box.width} ${element.box.height}`}>
      ...
      {ticks.map((tick, index) => {
        ...
        return (
          <g key={tick.ratio}>
            <path className="chart-grid" d={roughLinePath({ x: left, y: tick.y }, { x: right, y: tick.y }, `${element.id}:grid:${index}`, { segments: 6, wobble: 0.8 })} pathLength="1" style={{ ...drawStyle, opacity: axisProgress * 0.06 }} />
            <path className="chart-tick" d={roughLinePath({ x: left - 7, y: tick.y }, { x: left + 4, y: tick.y }, `${element.id}:tick:${index}`, { segments: 3, wobble: 0.7 })} pathLength="1" style={drawStyle} />
            <text x={left - 12} y={tick.y + 7} textAnchor="end">{revealText(label, progress[`${element.id}:y-label:${index}`] ?? 0)}</text>
          </g>
        );
      })}
      <path className="chart-axis" d={`${roughLinePath({ x: left, y: top - 5 }, { x: left, y: bottom }, `${element.id}:axis-y`, { segments: 6, wobble: 1.1 })} ${roughLinePath({ x: left, y: bottom }, { x: right + 4, y: bottom }, `${element.id}:axis-x`, { segments: 8, wobble: 1.1 })}`} pathLength="1" style={drawStyle} />
      {element.payload.series.map((series) => {
        ...
        const echoPath = pathFor(series.points, `${element.id}:${series.id}:echo`);
        ...
        return (
          <g key={series.id}>
            <path className="chart-line-shadow" d={echoPath} pathLength="1" style={{ strokeDasharray: 1, strokeDashoffset: 1 - pathProgress }} />
