...
14. Prefer normal line/box primitives and let the shared hand-drawn renderer add deterministic roughness. Use custom SVG paths for diagrams or irregular icons. Do not add scene-ID conditionals to `RecreationCanvas` or `RecreationStage`.
15. Register the scene once in `features/recreation/scenes/index.ts`. The history sidebar, gallery and `/scenes/<scene-id>` route should then appear automatically.
16. Add a scene structure test asserting the key content groups, diagrams, anchored annotations and intended choreography. For paged video scenes, assert page order, page-turn placement, and the semantic drawing order inside representative pages. Run scene validation and ensure no duplicate element IDs/orders, missing annotation/mark targets, invalid page references, invalid mark order or out-of-bounds boxes.
17. Compare the finished canvas against the reference at both full-page and detail level. Fix geometry/content errors in the new scene only. Do not change another scene to make the new image fit. If a missing capability is genuinely generic, extend the shared scene protocol/renderer with tests first, then use it from the new scene.
