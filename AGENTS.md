...
4. Create one new file under `features/recreation/scenes/<scene-id>.ts`. Use a canonical lowercase kebab-case scene ID.
5. Rebuild the image with HTML text and SVG geometry. **Do not use the source image as the page background.**
...
8. A single virtual pen is the invariant: no two dynamic elements may intentionally animate concurrently. For text, explicit source-image line breaks must be preserved. For underlines/highlights/circles, prefer semantic `targetId + match` marks so geometry is derived from rendered characters.
...
11. Add a scene structure test asserting the key Cue groups, Notes sections and diagrams. Run scene validation and ensure no duplicate element IDs/orders or out-of-bounds boxes.
12. Do not change another scene to make the new image fit. If a missing capability is genuinely generic, extend the shared scene protocol/renderer with tests first, then use it from the new scene.
